...
public class RoleService {
	 ...
	 * @param idJson
	 */
	public void listRoleForPageCheckBox(Role queryRole,
			PageObject<RoleDto> pageObject, String idJson) {
	...
	public String deleteRole(Role role) {
		...
		// if (!"0".equals(this.userRoleDao.countUserRoleByRole(role.getId())))
		// {
		roleDao.deleteObject(role);
		roleMenuDao.deleteRoleMenuByRoleId(role.getId());
		json.put("type", "success");
		// } else {
		// json.put("type", "nozero");
		// }
	...
	private boolean checked(Role role, Menu menu,
			List<String> selectedMenuIdList) {
	...
	private List<String> getSelectedMenuIdList(Role role) {
		...
		if (role != null) {
			List<RoleMenu> roleMenuList = roleMenuDao.listRoleMenuByRoleId(role
					.getId());
